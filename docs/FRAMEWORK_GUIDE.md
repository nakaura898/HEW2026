...
**レイヤー（Layer）** = 自分が「何者か」を表すフラグ
**マスク（Mask）** = 「何と衝突するか」を表すフラグ
...
```
プレイヤー: Layer=0x01, Mask=0x02
敵:         Layer=0x02, Mask=0x01

判定:
  プレイヤーのLayer(0x01) & 敵のMask(0x01) = 0x01 ≠ 0 → OK
  敵のLayer(0x02) & プレイヤーのMask(0x02) = 0x02 ≠ 0 → OK
  → 衝突する！
```
...
```
敵A: Layer=0x02, Mask=0x01
敵B: Layer=0x02, Mask=0x01

判定:
  敵AのLayer(0x02) & 敵BのMask(0x01) = 0x00 = 0 → NG
  → 衝突しない！
```
